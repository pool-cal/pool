    ...
    public partial class Form1 : Form
    {
        ...
        private void MessageConverter(string message, string check)
        {
            ...
            if (check.Equals("p@"))
            {
                ...
                _moveShapeSetting.Add(strKey[1], setting);      // 여기 에러
        ...
        private void SendMessage(string message, int check)
        {
            ...
            try
            {
                ...
                {
                    copy_client = _connectedClients[i];
                    ...
                    IPEndPoint ip = (IPEndPoint)copy_client.RemoteEndPoint;
                    string port = ip.Port.ToString();
                    string localIP = copy_client.LocalEndPoint.ToString();

                    byte[] bytes = Encoding.Default.GetBytes(localIP + "%" + _check + message + "%");

                    //_client.Send(bytes);
                    Console.WriteLine("ip; " + ip + _check + message);
        ...
        private void pictureBox1_MouseUp(object sender, MouseEventArgs e)
        {
            if (_isClicked)
            {
                ...
                // rect create
                //_moveShapeRect.X = _startPoint.X;
                //_moveShapeRect.Y = _startPoint.Y;
                //_moveShapeRect.Width = _endPoint.X - _startPoint.X;
                //_moveShapeRect.Height = _endPoint.Y - _startPoint.Y;
