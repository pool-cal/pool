...
namespace Client1
{
    public partial class Form1 : Form
    {
        ...
        private int _pointKey_Server = 1;       // 서버 키 
        private int _pointKey_Client = 1;       // 클라이언트 키
        // 이동에 필요한 변수
        ...
        private void MessageConverter(string message, string check, string port)
        {
            ...
            if (check.Equals("p") || check.Equals("p@"))
            {
                ...
                _moveShapeSetting_Server.Add(strKey[1], setting);

                _pointKey_Server = Convert.ToInt32(strKey[1]);
...
#region 안씀
// stream으로 tcp연결하고 받아온 데이터 객체 각각 변환 후 계속 뿌려주는 함수 (사용 x)
